...
func help(root *TreeNode,count int,sum int) bool {
	...
	ok := false
	if root.Left != nil {
		ok = help(root.Left,count + root.Val,sum)
		if ok {
			return true
		}
	}
	if root.Right != nil {
		ok = help(root.Right,count + root.Val,sum)
	}
	return ok
}
...
那么我们直接判断 sum 是否等于 val 即可（因为路径和已经确定，就是当前节点的值，
