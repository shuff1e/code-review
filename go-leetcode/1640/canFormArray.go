...
func canFormArray(arr []int, pieces [][]int) bool {
	...
	return help(arr,pieces,0,visited)
}

func help(arr []int,pieces [][]int,index int,visited []bool) bool {
	if index == len(arr) {
		return true
	}
	...
	for i := 0;i<len(pieces);i++ {
		if !visited[i] {
			...
			if j == len(word) {
				visited[i] = true
				if help(arr,pieces,index + len(word),visited) {
					return true
				}
				visited[i] = false
			}
